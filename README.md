...
- **Choose the printing** — every card can show its **oldest** (default), cheapest, priciest or
  a full-art version (borderless and showcase printings count), set under Settings → Default
  printing; picking a specific printing on a card pins it to that card.
